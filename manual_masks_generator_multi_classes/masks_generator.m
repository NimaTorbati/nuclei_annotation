...
for counter = 1:length(imagej_zips) %loop over images
   ...
   %Check if any new class has been added and keep the colors order same as
   %before
   if counter == 1
       uniqueStrokeColors1 = uniqueStrokeColors;%ignore it for the first iteration
   else
       if mean(uniqueStrokeColors1 ~= uniqueStrokeColors) ~= 1
           for i = 1:length(uniqueStrokeColors)
               if isempty(find(uniqueStrokeColors(i) == uniqueStrokeColors1))
                   uniqueStrokeColors1 = [uniqueStrokeColors1, uniqueStrokeColors(i)];
               end
           end
   ...
   color_code = [4278255360 4294901760 4294967295];
   for ii = 1:length(uniqueStrokeColors)
       ...
       for n_num=1:length(ROIs) % loop over rois in one image
           ...
           if sROI.nStrokeColor == uniqueStrokeColors(ii)
               color = sROI.nStrokeColor;
               c_ind = find(color == color_code);
       ...
       object_count_tot (c_ind + 1, counter) = nuc_class_count;
               %% for masks with diffent label for each object
               savepath_labelmask = strcat(results_path,'class_' , string(c_ind),'_annotations\','label_masks','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               ...
               savepath_labelmask = strcat(results_path,'class_' , string(c_ind),'_annotations\','label_masks_modify','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               ...
               savepath_binary = strcat(results_path,'class_' , string(c_ind),'_annotations\','mask_binary','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               ...
               for diskthick =1:10
                   ...
                   savepath_border = strcat(results_path,'class_' , string(c_ind),'_annotations\','nuclei_border\disk',string(diskthick),'\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
                   imwrite(border, savepath_border);
               end

               %% for mask removing borders like TMI paper
               savepath_binary_borderremoved = strcat(results_path,'class_' , string(c_ind),'_annotations\','mask_binary_without_border','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               ...
               weighted_maps_path = strcat(results_path,'class_' , string(c_ind),'_annotations\','weighted_maps','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               weighted_maps_erode_path = strcat(results_path,'class_' , string(c_ind),'_annotations\','weighted_maps_erode','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               ...
               savepath_binary_borderremoved = strcat(results_path,'class_' , string(c_ind),'_annotations\','mask_binary_without_border_erode','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               imwrite(uint8(mask_binary_without_border_erode),savepath_binary_borderremoved);

               %% for distance maps
               savepath_distance = strcat(results_path,'class_' , string(c_ind),'_annotations\','distance_maps','\', strcat(erase(raw_imgs(counter).name,'.png'),'.png'));
               ...
               original = imresize(original, [size_target size_target]);
               ...
               for i=1:length(ROIs)
                   ...
                   hold on
                   visboundaries(dum,'Color',color_opt{c_ind},'LineWidth', 1,'LineStyle', '-');
               end
               subplot(1,2,1);imshow(original);title({'cropped image'},'FontSize', 22);

               save_path_overlay = strcat(results_path,'class_' , string(c_ind),'_annotations\','overlay','\',raw_imgs(counter).name);
               % for white border (otherwise the borders will turn balck after saving)
               if color_opt{c_ind}=='#FFFFFF'
   ...
   sum_value4 = sum(cell2mat(data(:,5)));

   % Append the sum as the last row (with a label 'Total')
   data = [data; {'Total', sum_value1, sum_value2, sum_value3, sum_value4}];
